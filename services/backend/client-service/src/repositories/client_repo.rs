...
impl ClientRepo {
    ...
    pub async fn update_ticket_refund_status(
        &self,
        client_id: &str,
        ticket_cod: &str,
        refund_status: Option<&str>,
    ) -> Result<(), ClientRepoError> {
        let object_id = ObjectId::parse_str(client_id)
            .map_err(|_| ClientRepoError::InvalidObjectId(format!("Invalid ID: {}", client_id)))?;

        let update = if let Some(status) = refund_status {
            doc! { "$set": { "lista_bilete.$.refund_status": status } }
        } else {
            doc! { "$unset": { "lista_bilete.$.refund_status": "" } }
        };

        self.collection
            .update_one(
                doc! { "_id": object_id, "lista_bilete.cod": ticket_cod },
                update,
            )
            .await
            .map_err(|e| ClientRepoError::DatabaseError(e.to_string()))?;
