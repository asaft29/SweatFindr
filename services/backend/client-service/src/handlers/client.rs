...
use crate::models::client::{
    AddTicket, Client, ClientQuery, CreateClient, TicketBuyerInfo, TicketRef, UpdateClient,
};
use crate::services::event_service;
use crate::utils::error::{ClientApiError, map_authorization_error, map_event_service_error};
use crate::utils::links::{
    Response, build_filtered_client, build_simple_client, build_ticket_buyer_info, build_ticket_ref,
};
...
pub async fn add_ticket_to_client(
    ...
    Ok((
        StatusCode::CREATED,
        Json(build_simple_client(client, &state.base_url)),
    ))
}
...
pub async fn get_my_client(
    ...
    let user_email = get_user_email(&state, user_claims.user_id)
        .await
        .ok_or_else(|| ClientApiError::InternalError("Failed to get user email".to_string()))?;
...
pub async fn delete_my_account(
    ...
    auth_client.delete_user(delete_request).await.map_err(|e| {
        ClientApiError::InternalError(format!("Failed to delete user from auth service: {}", e))
    })?;
...
pub async fn get_client_by_ticket(
    ...
    Ok(Json(build_ticket_buyer_info(
        buyer_info,
        &ticket_code,
        &state.base_url,
    )))
